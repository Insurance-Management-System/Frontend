...
import { Search, Eye, Phone, Mail, MapPin, Calendar, Trash2 } from "lucide-react"
...
export default function AdminCustomers() {
  const { customers, purchasedPolicies, refresh } = useData()
  ...
  return (
    <div>
      ...
      <div className="card border-0 shadow-sm p-4 mt-4">
        <h2 className="h6 fw-semibold mb-3">System Users</h2>
        <div className="table-responsive">
...
function CustomerDetail({ customer, onClose }) {
  const { purchasedPolicies, claims, payments, getPolicy, confirmPolicyCancellation } = useData()
  ...
  async function handleConfirmCancellation(id) {
    setProcessingId(id)
    try {
      await confirmPolicyCancellation(id)
    } catch (err) {
      alert(err.message)
    } finally {
      setProcessingId("")
                      ...
                      <div className="d-flex align-items-center gap-2">
                        ...
                        {p.status === "Cancellation Requested" && (
                          <button
                            ...
                            onClick={() => handleConfirmCancellation(p.id)}
                          >
                            {processingId === p.id ? "..." : "Confirm Cancellation"}
                          </button>
                        )}
                      </div>
                    </div>
